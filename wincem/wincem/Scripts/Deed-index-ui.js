...
$(document).on("contextmenu", ".deedRecord", function (e) {
    $('#deleteLink').attr('href', config.baseURL + 'Owner/Delete/' + $(this).attr("id"));
    $('#editLink').attr('href', config.baseURL + 'Owner/Edit/' + $(this).attr("id"));
...
function deedSearch() {
    if ($('#searchString_deed').val().length > 2 || $('#searchString_deed').val() == '') {
        $.ajax({
            url: config.baseURL + 'Owner/SearchDeeds?SearchString=' + $('#searchString_deed').val() + '&number_of_records=' + $('#number_of_records_deed').val() + "&deed_number=" + $('#deed_number').is(':checked') + "&Owner_Name=" + $('#Owner_Name').is(':checked')
